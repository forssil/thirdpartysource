 ...
 private:
  void Initialize(COINIT init) {
    //hr_ = CoInitializeEx(NULL, init);
	  hr_ = CoInitializeEx(NULL, COINIT_APARTMENTTHREADED); //zhanghuabing
  }
...
public:
	...
	//zhanghuabing
	// should be called before Init Recoding/Playout
	virtual int32_t SetRecordingSampleRate(const uint32_t samplesPerSec);
	virtual int32_t SetPlayoutSampleRate(const uint32_t samplesPerSec);

	//zhanghuabing
	//Inquire the device samplerate
	virtual int32_t GetRecordingSampleRate(uint32_t& samplesPerSec);
	virtual int32_t GetPlayoutSampleRate(uint32_t& samplesPerSec);
...
private:  // WASAPI
	...
	IAudioEndpointVolume*					_ptrRenderVolume; //zhanghuabing
...
private:
	uint32_t m_nRecordingSampleRate;
	uint32_t m_nPlayoutSampleRate;
