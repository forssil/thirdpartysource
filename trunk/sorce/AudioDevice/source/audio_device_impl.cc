...
#endif
#include "dummy/audio_device_dummy.h"
#include "dummy/audio_device_utility_dummy.h"
                                                 ...
                                                 const AudioLayer audioLayer)
{
	...
	//todo:test for comment below line, zhanghuabing
    //WebRtcSpl_Init();
...
int32_t AudioDeviceModuleImpl::RecordingSampleRate(uint32_t* samplesPerSec) const
{
	...
	uint32_t sampleRate;
	uint32_t & recordingSampleRate = sampleRate;

	_ptrAudioDevice->GetRecordingSampleRate(recordingSampleRate);

	if (recordingSampleRate == -1)
	{
		WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to retrieve the sample rate");
		return -1;
	}

	*samplesPerSec = recordingSampleRate;
...
int32_t AudioDeviceModuleImpl::PlayoutSampleRate(uint32_t* samplesPerSec) const
{
	...
	uint32_t sampleRate;
	uint32_t & playoutSampleRate = sampleRate;

	_ptrAudioDevice->GetPlayoutSampleRate(playoutSampleRate);

	if (playoutSampleRate == -1)
	{
		WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to retrieve the sample rate");
		return -1;
	}

	*samplesPerSec = playoutSampleRate;
