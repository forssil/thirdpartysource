...
AudioDeviceWindowsCore::AudioDeviceWindowsCore(const int32_t id) :
	...
	_ptrRenderVolume(NULL), //Zhanghuabing
    _dmo(NULL),
    _mediaBuffer(NULL),
    ...
    _newMicLevel(0),
	m_nRecordingSampleRate(0), //zhanghuabing added
	m_nPlayoutSampleRate(0)
{
...
int32_t AudioDeviceWindowsCore::Terminate()
{
	...
	SAFE_RELEASE(_ptrRenderVolume); //Zhanghuabing
...
int32_t AudioDeviceWindowsCore::InitSpeaker()
{
	...
	//============================================
	//replace simpleAudio with endpoint volume
	//Zhanghuabing
	ret = _ptrDeviceOut->Activate(__uuidof(IAudioEndpointVolume),
		CLSCTX_ALL,
		NULL,
		reinterpret_cast<void **>(&_ptrRenderVolume));
	if (ret != 0 || _ptrRenderVolume == NULL)
	{
		WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
			"  failed to initialize the render volume");
		SAFE_RELEASE(_ptrRenderVolume);
		return -1;
	}
	//===============================================
	// webRTC original code
	/*
	IAudioSessionManager* pManager = NULL;
	ret = _ptrDeviceOut->Activate(__uuidof(IAudioSessionManager),
		CLSCTX_ALL,
...
int32_t AudioDeviceWindowsCore::SetSpeakerVolume(uint32_t volume)
{
	...
	hr = _ptrRenderVolume->SetMasterVolumeLevelScalar(fLevel, NULL);//zhanghuabing
...
int32_t AudioDeviceWindowsCore::SpeakerVolume(uint32_t& volume) const
{
	...
	hr = _ptrRenderVolume->GetMasterVolumeLevel(&fLevel); //zhanghuabing
    _volumeMutex.Leave();
    EXIT_ON_ERROR(hr);
...
int32_t AudioDeviceWindowsCore::PlayoutIsAvailable(bool& available)
{
	...
	// don't call InitPlayout at the beginning -- zhanghuabing
...
int32_t AudioDeviceWindowsCore::InitPlayout()
{
	...
	////zhanghuabing
	////add WAVEFORMATEXTENSIBLE  for query and set device samplerate 
	//WAVEFORMATEXTENSIBLE* pWfxOut_extens = NULL;
	//WAVEFORMATEXTENSIBLE Wfx_extens = { 0 };
	//PWAVEFORMATEXTENSIBLE pWfxClosestMatch_extens = NULL;
	...
	////zhanghuabing
	//Wfx_extens.Format.wFormatTag = WAVE_FORMAT_PCM;
	//Wfx_extens.Format.wBitsPerSample = 16;
	//Wfx_extens.Format.cbSize = sizeof(WAVEFORMATEXTENSIBLE)-sizeof(WAVEFORMATEX);
	//Wfx_extens.SubFormat = KSDATAFORMAT_SUBTYPE_PCM;
	//Wfx_extens.Samples.wValidBitsPerSample = 16;
	//Wfx_extens.Samples.wReserved = 0;
	//Wfx_extens.Samples.wSamplesPerBlock = 0;
    ...
    for (int freq = 0; freq < sizeof(freqs)/sizeof(freqs[0]); freq++)
    {
        for (int chan = 0; chan < sizeof(_playChannelsPrioList)/sizeof(_playChannelsPrioList[0]); chan++)
        {
			...
			//zhanghuabing
			//========================
			//Wfx_extens.Format.nChannels = _playChannelsPrioList[chan];
			//Wfx_extens.Format.nSamplesPerSec = freqs[freq];
			//Wfx_extens.Format.nBlockAlign = Wfx.nChannels * Wfx.wBitsPerSample / 8;
			//Wfx_extens.Format.nAvgBytesPerSec = Wfx.nSamplesPerSec * Wfx.nBlockAlign;
			//
			//hr = _ptrClientOut->IsFormatSupported(
			//	AUDCLNT_SHAREMODE_EXCLUSIVE,
			//	(WAVEFORMATEX* )&Wfx_extens,
			//	&pWfxClosestMatch);
			//========================
    ...
    if (hr == S_OK)
    {
		...
		//zhanghuabing
		//m_nPlayoutSampleRate = Wfx_extens.Format.nSamplesPerSec;
		m_nPlayoutSampleRate = _playSampleRate;

		//_playAudioFrameSize = Wfx_extens.Format.nBlockAlign;
		//_playBlockSize = Wfx_extens.Format.nSamplesPerSec / 100;
		//_playSampleRate = Wfx_extens.Format.nSamplesPerSec;
		//_devicePlaySampleRate = Wfx_extens.Format.nSamplesPerSec; // The device itself continues to run at 44.1 kHz.
		//_devicePlayBlockSize = Wfx_extens.Format.nSamplesPerSec / 100;
		//_playChannels = Wfx_extens.Format.nChannels;
		//========================================================
	...
	// zhanghuabing
	//hr = _ptrAudioBuffer->SetPlayoutSampleRate(_playSampleRate);
	//hr = _ptrAudioBuffer->SetPlayoutChannels((uint8_t)_playChannels);
	...
	//	0,                                    // periodicity
	//	&Wfx_extens.Format,						// selected wave format
	//	NULL);                                // session GUID
...
int32_t AudioDeviceWindowsCore::InitRecording()
{
    ...
    if (hr == S_OK)
    {
		...
		//zhanghuabing
		m_nRecordingSampleRate = _recSampleRate = Wfx.nSamplesPerSec;
...
//zhanghuabing
int32_t AudioDeviceWindowsCore::SetRecordingSampleRate(const uint32_t samplesPerSec)
{
	m_nRecordingSampleRate = samplesPerSec;
	return 0;
}

int32_t AudioDeviceWindowsCore::SetPlayoutSampleRate(const uint32_t samplesPerSec)
{
	m_nPlayoutSampleRate = samplesPerSec;
	return 0;
}

//Inquire the device samplerate
int32_t AudioDeviceWindowsCore::GetRecordingSampleRate(uint32_t& samplesPerSec)
{
	samplesPerSec = m_nRecordingSampleRate;
	return 0;
}

int32_t AudioDeviceWindowsCore::GetPlayoutSampleRate(uint32_t& samplesPerSec)
{
	samplesPerSec = m_nPlayoutSampleRate;
	return 0;
}
