	  ...
	  if (m_bInit)
	  {
		  ...
		  if (m_AECData.bNROn_)
		  {
			if (!aShareData.bRNNOISEVad_ && !aShareData.nNearVAD_) { // || aShareData.ChannelIndex_ != 0
              ...
              float erle = 0.f;
			  float re,im,sum;
              for (CAUDIO_U32_t i = 2; i < m_AECData.nLengthFFT_ / 4; i++)
              {
                  {
					  re = m_AECData.pDesireFFT_[2 * i];
					  im = m_AECData.pDesireFFT_[2 * i + 1];
                      sum = re*re + im*im;
                      //Y[i] = sum;
					  erle += sum;
					  
					  re = m_AECData.pRefferFFT_[2 * i];
					  im = m_AECData.pRefferFFT_[2 * i + 1];
                      sum = re*re + im*im;
                     // X[i] = sum;
                      x_psd += sum;

					  re = m_AECData.pErrorFFT_[2 * i];
					  im = m_AECData.pErrorFFT_[2 * i + 1];
                      sum = re*re + im*im;
                      E += sum;
                  }
			...
			if (m_AECData.fErle_ > 8 && (x_psd > 0.00001 || m_AECData.nFarVAD_) && E_nlp < 0.000002) { // E_nlp < -62dB
				...
				extragain= 0.01; // double talk suppression
			}
			if (first_render_>0 && first_render_<30 && first_render_his_< first_render_ && m_AECData.fErle_ > 3) {
              ...
              {
				m_AECData.pErrorFFT_[i] *= extragain;
              }
