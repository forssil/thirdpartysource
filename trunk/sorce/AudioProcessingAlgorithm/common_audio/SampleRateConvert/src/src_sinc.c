...
#define	INV_FP_ONE				0.000244140625
...
//zhanghuaing
static double fp_to_double_LookupTable[4096];

//zhanghuabing
static inline increment_t
fp_fraction_part_own(increment_t x)
{	//return ((x) & ((((increment_t) 1) << SHIFT_BITS) - 1)) ;
	return ((x)& 0x00000fff);
} /* fp_fraction_part_own */

//zhanghuabing
//for optimize
/*
extern void generateLookupTable(void)
{
	for (int i = 0; i < 4096; i++)
	{
		fp_to_double_LookupTable[i] = i * INV_FP_ONE;
	}
}
*/
...
{	
	...
	return fp_to_double_LookupTable[fp_fraction_part_own(x)];
} /* fp_to_double */
