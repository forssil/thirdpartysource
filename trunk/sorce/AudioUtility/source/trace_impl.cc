...
#ifdef _WIN32
#include "source/win/trace_win.h"
#else
#include "source/trace_posix.h"
#endif  // _WIN32
...
TraceImpl::TraceImpl()
  ...
  thread_.Start(tid);
  message_pool=new
	  char[WEBRTC_TRACE_MAX_MESSAGE_SIZE*WEBRTC_TRACE_NUM_ARRAY*WEBRTC_TRACE_MAX_QUEUE];
  memset(message_pool,0,WEBRTC_TRACE_MAX_MESSAGE_SIZE*WEBRTC_TRACE_NUM_ARRAY*WEBRTC_TRACE_MAX_QUEUE*sizeof(char));
  for (int m = 0; m < WEBRTC_TRACE_NUM_ARRAY; ++m) {
    for (int n = 0; n < WEBRTC_TRACE_MAX_QUEUE; ++n) {
      message_queue_[m][n] = message_pool+m*WEBRTC_TRACE_MAX_QUEUE*WEBRTC_TRACE_MAX_MESSAGE_SIZE+n*WEBRTC_TRACE_MAX_MESSAGE_SIZE;
    }
  }
}
