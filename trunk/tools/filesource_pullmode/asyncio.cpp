...
CAsyncIo::CAsyncIo(CAsyncStream *pStream)
         : m_hThread(NULL),
           ...
           m_bWaiting(FALSE),
           m_pStream(pStream),
		   m_bStartAsyncRead(TRUE),
		   m_bCanNotCreateThread(FALSE),
		   m_llPos(0)
{
...
CAsyncIo::StartThread(void)
{
	...
	if (m_bCanNotCreateThread)
	{
		return S_OK;
	}
	if (m_bStartAsyncRead)
	{
		m_pStream->Reset();
		m_bStartAsyncRead = FALSE;
	}
