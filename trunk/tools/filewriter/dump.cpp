...
CFactoryTemplate g_Templates[]= {
    L"Codyy Dump", &CLSID_Dump, CDump::CreateInstance, NULL, &sudDump
};
int g_cTemplates = 1;
...
HRESULT CDumpInputPin::CheckMediaType(const CMediaType *pmt)
{
	...
	WAVEFORMATEX *pwfex;
	pwfex = (WAVEFORMATEX *)const_cast<CMediaType *>(pmt)->AllocFormatBuffer(sizeof(WAVEFORMATEX));

	SWavFileHead *const wavfilehead = m_pDump->m_pWavFileHead;
	
	wavfilehead->RIFF[0] = 'R';
	wavfilehead->RIFF[1] = 'I';
	wavfilehead->RIFF[2] = 'F';
	wavfilehead->RIFF[3] = 'F';
	wavfilehead->data[0] = 'd';
	wavfilehead->data[1] = 'a';
	wavfilehead->data[2] = 't';
	wavfilehead->data[3] = 'a';
	wavfilehead->WAVEfmt_[0] = 'W';
	wavfilehead->WAVEfmt_[1] = 'A';
	wavfilehead->WAVEfmt_[2] = 'V';
	wavfilehead->WAVEfmt_[3] = 'E';
	wavfilehead->WAVEfmt_[4] = 'f';
	wavfilehead->WAVEfmt_[5] = 'm';
	wavfilehead->WAVEfmt_[6] = 't';
	wavfilehead->WAVEfmt_[7] = ' ';

	wavfilehead->noUse = 0x00000010;
	wavfilehead->FormatCategory = 1;
	wavfilehead->SampleRate = pwfex->nSamplesPerSec;
	wavfilehead->SampleBytes = pwfex->nAvgBytesPerSec;
	wavfilehead->BytesPerSample = pwfex->nBlockAlign;
	wavfilehead->NBitsPersample = pwfex->wBitsPerSample;
	wavfilehead->NChannels = pwfex->nChannels;
	wavfilehead->FileLength = 0;
	wavfilehead->RawDataFileLength = 0;
	...
	m_pcFileName(NULL)
{
	...
	if (m_pWavFileHead == NULL)
	{
		if (phr)
			*phr = E_OUTOFMEMORY;
		return;
	}
	memset(m_pWavFileHead, 0, sizeof(SWavFileHead));
	m_pWavFileHead->BytesPerSample = 4; // default value
}
...
HRESULT CDump::OpenFile()
{
	...
	m_pcFileName = ConvertLPWSTRToLPSTR(m_pFileName);

	m_pWavFileOp = new CWavFileOp(m_pcFileName, "wb");
	if (m_pWavFileOp->m_FileStatus == -2)
	{
		delete m_pWavFileOp;
		m_pWavFileOp = NULL;
		return E_FAIL;
	...
	if (m_pWavFileOp)
	{
		int ret = m_pWavFileOp->WriteHeader(*m_pWavFileHead);
		if (0 != ret)
		{
			return E_FAIL;
		}
...
HRESULT CDump::Write(PBYTE pbData, LONG lDataLength)
{
	...
	if (m_pWavFileOp)
	{
		int ret = m_pWavFileOp->WriteSample((short*)pbData, lDataLength/2);
		if (0 != ret)
		{
			return E_FAIL;
		}
	}
	m_dwFileLength += lDataLength;

    return S_OK;
}

HRESULT CDump::UpdateFileHeader(void)
{
	if (m_pWavFileOp)
	{
		int ret = m_pWavFileOp->UpdateHeader(m_pWavFileHead->NChannels, m_dwFileLength / m_pWavFileHead->BytesPerSample);
		if (0 == ret)
		{
			return NOERROR; 
		}
...
char* CDump::ConvertLPWSTRToLPSTR(LPWSTR lpwszStrIn)
{
    LPSTR pszOut = NULL;
	if (lpwszStrIn != NULL)
	{
